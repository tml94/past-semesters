...
%%%%%%%%%%%%%%%%%%%% Functions %%%%%%%%%%%%%%%%%%%%%%%%
%\section*{Functions}
%\textbf{Bring little whiteboards for the students to do their ALEKS work on every day. This way they can keep their notebooks for notes and can work problems in a way that I can see when they need help.\\}
%
%\noindent
%Prior to starting math lecture on page 1 of the students notes, I want them to have two lists\\
%
%\noindent
%\textbf{Strategies:}
%\begin{enumerate}
%	\item Identify input/output (always do this, write it at the beginning of every problem, even if the problem has defined it, write it again)
%	\item Make a table	
%	\item Identify an appropriate domain
%	\item Formula
%	\item Graph
%\end{enumerate}
%
...
%	\item Use: 
%		\begin{enumerate}
%			\item Your brain — what you already know
...
%In Tucson, the total sales tax is approximately 8.2\%. How do you calculate the total cost of an item (including tax), given a particular ticket price?
%	\bigskip\\
%		\hangindent=1.cm \hangafter=2
%			Students work on this with their table\\
...
%		\begin{enumerate}
%			\item \textbf{Definition} (A function is a relationship between two variables, an input variable (often $x$) and an output variable (often $y$) in such a way that for every value of the input variable there is exactly one corresponding value of the output variable.)
%			\item \textbf{Input} (or independent) variable 
%			\item \textbf{Output} (or dependent) variable
%			\item \textbf{Representations} (equations, graphs, tables)
%			\item \textbf{Domain} (the set of all values of the input variable for which the funciton is defined in the real number system and in the case of an application problem in which the context makes sense) 
%			\item \textbf{Range} (the set of values of the dependent variable (easiest to identify from a graph))
%			\item \textbf{Domain Restrictions} (Even Radical Functions, Odd Radical Functions, Fractions, Context)
%		\end{enumerate}
%		\noindent
...
%		Have students work on this at their tables first.\\
...
%		Students work on this at their tables.\\
...
%\section*{Graphs}
%Start with a warm-up to help students get an appropriate viewing window. Given the function without the context, I am going to have the students find the$\ldots$ $y$-intercept and the $x$-intercepts.
...
%\noindent
%DO THIS PROBLEM WITH THE STUDENTS!\\

%\noindent
%Things to talk about as we graph the equation:
%\begin{itemize}
%	\item Profit= (Revenue) - (Cost)
%	\item Appropriate viewing window: I can see all important features
%	\item Input:
%	\item Output:
%	\item Table feature (how does this help us?)
%\end{itemize}

%\noindent
%Things to talk about after we graph the equation:
%\begin{itemize}
%	\item Profit is increasing on the interval:
%	\item Profit is decreasing on the interval:
%	\item Domain:
%	\item Range:
%	\item Profit is positive on:
%	\item Profit is:
%	\item Profit is maximized: (using the calculate feature) 
%\end{itemize}
...
%	\item x-intercept(s): points for which $f(x)=0$, graph intersects the $x-axis$
%	\item y-intercept: point the graph intersects the $y-axis$ ($y$-value for when $x=0$)
%\end{itemize}
...
%
%\noindent
%\textbf{Slope:} positive slope, negative slope, horizontal line, vertical line, units of slope (units of $f(x)$ divided by units of x, per), finding the slope
%
%\noindent
...
%	\item Determine an equation in slope-intercept form for the linear function graphed below. Identify the intercepts.\\ \begin{center} \includegraphics[scale=0.15]{Lineargraph} \end{center}
%\end{enumerate}
...
%\end{itemize}
%
%\subsection*{Computational Problem for Domain intuition building}
%Consider the following equations: \[f(x)=3x-1 \text{ and } g(x)=\sqrt{5-2x} \]
%Find:
%\begin{enumerate}
%	\item The domain of $f(x)$
%	\item The domain of $g(x)$
%	\item The quotient $\frac{g(x)}{f(x)}$
%	\item The domain of $\frac{g(x)}{f(x)}$
%\end{enumerate}
%
...
%	\item How do we find the domain for each of these?
%\end{itemize}
%
...
%	\item Table 2: $(f+g)(x)$
%	\item Table 3: $(f-g)(x)$
%	\item Table 4: $(f \cdot g)(x)$
%	\item Table 5: $\left(\frac{f}{g}\right)(x)$
%	\item Table 6: $\left(\frac{g}{f}\right)(x)$
%	\item Table 7: $(f \circ g)(x)$
%	\item Table 8: $(g \circ f)(x)$
%	\item Table 9: $\left(\frac{h}{k}\right)(x)$
%	\item Table 10: $\left(\frac{k}{h}\right)(x)$	\item Table 11: $(h \circ k)(x)$
%	\item Table 12: $(k \circ h)(x)$
%\end{itemize}
%
%
%\subsection*{Example 1}
...
%
%\noindent
%Comments: \textit{This is a rote example to practice the technique of solving for $x$. As this is a new technique that the students won't be familiar with. We should graph the function as well to show how inverse function work (ie reflecting over the line y=x). With this example we should also evaluate $f(f^{-1}(x)$ and $f^{-1}(f(x))$ }
%
...
%We begin with a quick review of an activity geared towards students discovering different aspects of the polynomial functions some basic graphs i.e. $x$, $x^2$, $x^3$, $x^4$, and $x^5$, and then more advanced graphs to help the students understand what multiplicity gives us in the graphs, as well as how different multiplicities affect zeros. The students will do this for homework and I will have them discuss the questions before we talk about things to know about polynomial functions. \\
%
...
%	\item $y=\frac{1}{20}x(x+5)(x+2)(x-5)$ \includegraphics[scale=0.03]{quarticpractice.png}
%	\item $y=\frac{-1}{3}(x-3)(x+3)(x+1)$ \includegraphics[scale=0.05]{cubicpractice.png}
%	\end{enumerate}
...
%%%%%%%%% Rational Functions-Hannah Taught this Section Summer 2018 %%%%%%%%%%%%
...
%What is the formula for compound interest if it is compounded annually? $n$ times per year? continuously? \textit{$A=P(1+\frac{n}{t})^{nt}$, $A$ is the final amount, $P$ is the Principle (initial) amount, $n$ is the number of times per year. $A=Pe^{rt}$}\\
%
%\subsection*{Example 1}
